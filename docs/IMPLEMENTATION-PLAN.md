...
A structured plan to build the Myndulon AI chatbot widget from the current state to a fully functional MVP.
...
- [x] Create `services/qdrant_client.py`:
  ...
  - Create collection `myndulon_embeddings` if not exists
...
- Global `MyndulonWidget.init()` API for easy integration
